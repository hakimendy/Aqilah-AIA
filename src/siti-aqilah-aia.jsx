...
const Styles = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    /* ── NAV ── */
    .nav{position:fixed;top:0;left:0;right:0;z-index:200;background:rgba(255,253,249,0.97);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border-bottom:1px solid rgba(200,16,46,0.08);display:flex;align-items:center;justify-content:space-between;padding:0 5%;height:72px;transition:box-shadow 0.3s;}
    ...
    /* Dropdown */
    .nav-dropdown{position:absolute;top:calc(100% + 10px);left:50%;transform:translateX(-50%);background:#fff;border:1px solid rgba(200,16,46,0.1);border-radius:14px;box-shadow:0 16px 48px rgba(26,26,46,0.12),0 4px 12px rgba(26,26,46,0.06);min-width:220px;padding:0.5rem;opacity:0;visibility:hidden;transform:translateX(-50%) translateY(-8px);transition:opacity 0.18s,transform 0.18s,visibility 0.18s;z-index:300;}
    ...
    /* Mobile drawer */
    .nav-drawer{position:fixed;top:72px;left:0;right:0;bottom:0;z-index:150;background:rgba(255,253,249,0.99);backdrop-filter:blur(16px);overflow-y:auto;transform:translateX(100%);transition:transform 0.3s cubic-bezier(0.4,0,0.2,1);padding:1.5rem 5% 3rem;}
    ...
    <>
      ...
      <nav className={`nav${scrolled?" scrolled":""}`} role="navigation" aria-label="Main navigation" ref={navRef}>
        ...
        <div className="nav-right">
          <a href={FINANCE_URL} target="_blank" rel="noopener noreferrer" className="nav-btn-tracker" aria-label="Open Finance Tracker (opens in new tab)">
            📊 Finance Tracker
          </a>
          <button className="nav-btn-consult" onClick={()=>{scroll("contact");closeAll();}}>Free Consultation</button>
